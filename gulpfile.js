...
function imageMin() {
  ...
  return gulp.src(folder_from.images).pipe(newer(out)).pipe(imagemin()).pipe(gulp.dest(out));
}
...
function copyJStoDistOnChange(done) {
  ...
  var js_utils = {
    js: [folder_from.js + "utils/colors.js"],
  };

  var dummyFiles = {
    pdf: [folder_from.pdf + "dummy.pdf"],
  };
  ...
  // Копирование product-management.js
  gulp.src(folder_from.js + "product-management.js").pipe(gulp.dest(out));

  gulp
    .src([folder_from.js + "app.js"])
    .pipe(sourcemaps.init())
    .pipe(concat("app.js"))
    .pipe(gulp.dest(out))
    .pipe(
...
// default task
gulp.task("default", gulp.series(html, imageMin, css, copyJStoDistOnChange, "watch"));

// build
gulp.task("build", gulp.series(html, imageMin, css, copyJStoDistOnChange, javascript));
...
gulp.task("doc_sass", function () {
  return gulp
    .src("documentation/assets/scss/**/*.scss")
    .pipe(sourcemaps.init({ loadMaps: true }))
    .pipe(sass().on("error", sass.logError))
    .pipe(sourcemaps.write("maps"))
    .pipe(
