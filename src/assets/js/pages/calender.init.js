(function ($) {
  ...
  if (window.FullCalendar) {
    ...
    $('#fc_calendar').fullCalendar({
      ...
      drop: function (date, allDay) { // this function is called when something is dropped
        ...
        $('#fc_calendar').fullCalendar('renderEvent', copiedEventObject, true);
    ...
    $('#fc_calendar').fullCalendar('renderEvent', test, true);
  }
  else {
    alert("Calendar plugin is not installed");
  }

  $("#createEventForm").find(".form-control").on("change", function(){
    if($(this).val() === ""){
  ...
  $("#createEventForm").on("submit", function(e){
    ...
    if(!validated) return
    $('#fc_calendar').fullCalendar('renderEvent', obj, true);
    $(this).find(".floating-label").removeClass("enable-floating-label");
    $(this)[0].reset();
