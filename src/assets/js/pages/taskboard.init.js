  ...
  $(formdata).each(function (index, obj) {
    if (obj.name === 'validation-taskname') {
      titleHTML = '<div class="inner-header-title mb-2 fs-15">' +
        '<h6 class="fs-15 fw-semibold mb-0"><a href="#" data-bs-toggle="modal" data-target="#task-detail-modal" class="text-truncate text-dark">' + obj.value + '</a></h6>' +
        '<div class="dropdown">' +
        '<a href="#" class="btn-icon btn-icon-sm btn-icon-soft-primary dropdown-toggle edit-field-icon text-center" data-bs-toggle="dropdown" aria-expanded="false">' +
...
function generateBoardHTML(form, boardCount) {
  ...
  $(formdata).each(function (index, obj) {
    ...
    if (obj.name === "validation-boardname") {
      boardName = obj.value;
      headerTitle += '<h5 class="task-header">'+ obj.value +'<button class="btn avatar avatar-xs me-0 waves-effect waves-light addTask" data-bs-toggle="modal" data-target="#add-task-modal">' +
